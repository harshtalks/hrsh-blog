...
Now this may seem like a good normal function that takes care of everything related to coupon code. But it is not DRY. it is not DRY because of the code, the code is fine. It is because of the comments. We have created two sources of truth, the code and the comments. This is a classic example of duplication and is often termed as code smell (Clean Code 101). Anytime, you change the code, you have to change the comments as well. If you forget to do so, the comments will become outdated and misleading. This is how it breaks the DRY principle.
...
## Conclusion
- Avoid premature reusability, It is okay to refactor duplicate code and create knowledge abstraction in the code, but take your time to figure out all the requirements beforehand.
- Typescript is going to be a great help in creating reusable components and functions. There are lot of type helpers bundled with typescript that can help you create reusable types, components and functions.
- Avoid using HOCs as much as you can, Do not bring them to the new codebase. But if you are working on a legacy codebase with existing HOCs, it's better to not bring more patterns as HOCs probably will fit right to your team's mental model.
