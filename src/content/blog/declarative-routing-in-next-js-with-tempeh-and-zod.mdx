...
- `options` is a required argument in the `createRoute` function, it can be used to pass some additional options to the route config. Here we have passed `internal: true` which means that this route is an internal route and does not require any base route. For an external route such as an api, you can pass `internal: false` and provide a base route in the same object.
